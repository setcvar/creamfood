...
function Notification (message, color, size)
...
function runcmd (command) ICommand(command) end
...
local function Freeze ( name )
    ...
    local function Freeze ( instance )
        if instance:IsA("BasePart") and value.Anchored == false then
...
local function Thaw ( name )
    ...
    local function Thaw ( instance )
        if instance:IsA("BasePart") and value.Anchored == true then
...
local function Bhop ( )
    ...
    getgenv().bhop = game:GetService ( "RunService" ).Stepped:Connect(function()
        if game.Players.LocalPlayer.Character.Humanoid.FloorMaterial ~= Enum.Material.Air then ICommand ( "jump" ) end
    end)
...
local function addplugin (name)
    if not loadfile then Notification ("Your exploit doesn't have loadfile", Color3.fromRGB(200,0,0), Enum.FontSize.Size18) return end
    ...
    if plugin then
        ...
        Notification (plugin.Name .. " loaded successfully", Color3.fromRGB(100,200,100), Enum.FontSize.Size18)
    else
        Notification ("Couldn't find plugin!", Color3.fromRGB(200,0,0), Enum.FontSize.Size18)
    end
end

local function removeplugin (name)
    if table.find ( plugins, name ) then
        table.remove (plugins, table.find (plugins, name))
        local plugin = loadfile(name)()
        for index, value in pairs (plugin.Commands) do
            removecmd (index)
        end
    else
        Notification ("Couldn't find plugin! Did you put the right name?", Color3.fromRGB(200,0,0), Enum.FontSize.Size18)
    end
end

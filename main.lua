...
function WalkTo(object:Part)
	...
	local success, result = pcall(function()
		path:ComputeAsync(CHARACTER.HumanoidRootPart.Position, object.Position)
	end)
	...
	table.insert (commands.walkbot.events, blocked)
	...
	if not reached then
		reached = CHARACTER.Humanoid.MoveToFinished:Connect(function(reached)
			if reached and nextindex < #waypoints then
				...
				for index2, value2 in ipairs (commands.walkbot.events) do
					value2:Disconnect()
				end
				table.clear (commands.walkbot.events)
			end
		end)
		table.insert (commands.walkbot.events, reached)
	end
	nextindex = 2
	CHARACTER.Humanoid:MoveTo (waypoints[nextindex].Position)
